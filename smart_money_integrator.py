...
try:
    from sms_parser.core_parser import SMSTransactionParser
...
class SmartMoneyIntegrator:
    ...
    def process_bank_sms(self, sms_text: str, sender: str) -> Dict:
        ...
        try:
            # Parse SMS
            if self.sms_parser:
                transaction_data = self.sms_parser.parse_transaction_sms(sms_text, sender)
                if not transaction_data:
                    return {"status": "failed", "error": "SMS parsing failed"}
            else:
                # Fallback manual parsing
                transaction_data = self.manual_sms_parse(sms_text)
            
            # Categorize using ML
            if self.ml_categorizer and transaction_data:
                merchant = transaction_data.get('merchant', 'Unknown')
                description = transaction_data.get('description', sms_text)
